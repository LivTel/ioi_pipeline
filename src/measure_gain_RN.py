...
if __name__ == "__main__":
    ...
    group1.add_option('--l', action='store', dest='maxLinADU', default=5000, type=float, help='maximum ADU to consider for linear fit')
    ...
    for s1 in range(100, 1900, 50):
        for s2 in range(100, 1900, 50):
            means_average       = []
            shot_and_read_noise = []
            ## section files
            s1_lo = int(s1)
            ...
            for i in range(0, len(f_r1_data), 1):
                f_sect_r1 = f_r1_data[i][s1_lo:s1_hi, s2_lo:s2_hi] + np.mean(f_r1_rates[i][s1_lo:s1_hi, s2_lo:s2_hi])   # mean otherwise we add another bit of readout noise!
                f_sect_r2 = f_r2_data[i][s1_lo:s1_hi, s2_lo:s2_hi] + np.mean(f_r2_rates[i][s1_lo:s1_hi, s2_lo:s2_hi])   # mean otherwise we add another bit of readout noise!
