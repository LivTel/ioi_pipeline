...
def estimate_location(a, scale, norm=None, axis=0, initial=None,
                      maxiter=30, tol=1.0e-06, logger=None):
    """
    ...
    last_n_converged_values = None
    for iter in range(maxiter):
        W = norm.weights((a-mu)/scale)
        nmu = np.sum(W*a, axis) / np.sum(W, axis)
        
        n_values = len(np.less(np.fabs(mu - nmu), scale * tol).flatten())
        n_converged_values = len(np.where(np.less(np.fabs(mu - nmu), scale * tol) == True)[0])
        if logger is not None:
            logger.info("[statsmodels.robust.norms.estimate_location] Iteration " + str(iter+1) + " of " + str(maxiter)) 
            logger.info("[statsmodels.robust.norms.estimate_location] " + str(n_converged_values) + " values converged of " + str(n_values))        
        
        if np.alltrue(np.less(np.fabs(mu - nmu), scale * tol)):         # OK!
            logger.info("[statsmodels.robust.norms.estimate_location] Converged all values. Ending.")
            break
        #elif last_n_converged_values == n_converged_values:             # break out of loop if no more values converged
        #    logger.info("[statsmodels.robust.norms.estimate_location] No more values converged this iteration. Ending.")
        #    break
        elif iter==maxiter-1:                                           # break out of loop if reached last iter
            logger.info("[statsmodels.robust.norms.estimate_location] Reached maximum iterations. Ending.")
            break
        else:
            mu = nmu
            last_n_converged_values = n_converged_values 
    return nmu, np.less(np.fabs(mu - nmu), scale * tol)         # RMB - the latter argument is a boolean array describing if each pixel converged.
